...
func prepareDb() (*gorm.DB, error) {
	...
	if err != nil {
		log.Println(fmt.Sprintf("[DB] couldn't connect to db : %s", err))
		return nil, err
	}

	err = database.AutoMigrate(&Post{})
	if err != nil {
		log.Println(fmt.Sprintf("[DB] couldn't create new table : %s", err))
		return nil, err
	}
...
func createPost(database *gorm.DB, post Post) error {
	...
	if tx.Error != nil {
		log.Println(fmt.Sprintf("[DB] couldn't insert new post : %s", tx.Error))
		return tx.Error
	}
...
func createPosts(database *gorm.DB, posts []Post) error {
	...
	if tx.Error != nil {
		log.Println(fmt.Sprintf("[DB] couldn't insert new posts : %s", tx.Error))
		return tx.Error
	}
...
func updatePostScore(database *gorm.DB, post Post, score int64) error {
	...
	if tx.Error != nil {
		log.Println(fmt.Sprintf("[DB] couldn't find the post to be updated : %s", tx.Error))
		return tx.Error
	}

	tx = database.Model(&p).Update("Score", score)
	if tx.Error != nil {
		log.Println(fmt.Sprintf("[DB] couldn't update the post : %s", tx.Error))
		return tx.Error
	}
